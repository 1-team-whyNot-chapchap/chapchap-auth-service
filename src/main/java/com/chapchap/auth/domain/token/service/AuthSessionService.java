...
public class AuthSessionService {
    ...
    // 일반 사용자 Refresh Token 재발급 성공 시 세션의 유휴 만료 시작 연장
    public void extendUserIdleExpiration(
        AuthSession authSession
    ) {

        // 관리자 세션은 Sliding Session 정책을 적용하지 않는다.
        if (authSession.getSessionType() != SessionTypePolicy.USER) {
            return;
        }

        LocalDateTime now = LocalDateTime.now();

        // 현재 시각 기준 14일을 연장하되 절대 만료 시작을 넘지 않도록 계산
        LocalDateTime idleExpiresAt = authSessionPolicy
                                      .calculateExtendedUserIdleExpiresAt(
                                          now,
                                          authSession.getAbsoluteExpiresAt()
                                      );

        // 마지막 정상 사용 시각과 새로운 유휴 만료 시각 반영
        authSession.extendIdleExpiration(
            now,
            idleExpiresAt
        );
    }
